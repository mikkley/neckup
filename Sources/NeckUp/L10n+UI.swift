...
/// UI 层文案（岛/引导/设置/菜单栏）。语言解析与游戏文案见 NeckUpCore/L10n.swift。
extension L10n {
    ...
    public static var permBody: String {
        switch resolved {
        case .zhHans, .system: "NeckUp 只读取 AirPods 的头部姿态数据，\n所有记录只保存在本机，绝不上传。"
        case .en: "NeckUp only reads head-motion data from your AirPods.\nEverything stays on this Mac — nothing is uploaded."
        case .ja: "NeckUp が読み取るのは AirPods の頭部モーションのみ。\n記録はすべて本機に保存され、送信されません。"
        case .ko: "NeckUp은 AirPods의 머리 움직임 데이터만 읽습니다.\n모든 기록은 이 Mac에만 저장되며 업로드되지 않습니다."
        }
    }
    ...
    public static var settingsWindowTitle: String {
        switch resolved {
        case .zhHans, .system: "NeckUp 设置"
        case .en: "NeckUp Settings"
        case .ja: "NeckUp 設定"
        case .ko: "NeckUp 설정"
        }
    }
