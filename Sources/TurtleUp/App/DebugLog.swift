...
/// 文件在 ~/Library/Application Support/TurtleUp/debug.log，超 512KB 截断头部。
enum DebugLog {
    private static let queue = DispatchQueue(label: "turtleup.debuglog")
    private static let maxBytes = 512 * 1024
    private static let keepBytes = 256 * 1024

    static var fileURL: URL {
        let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("TurtleUp", isDirectory: true)
    ...
    static func exportToDesktop() -> URL? {
        ...
        let dest = desktop.appendingPathComponent("TurtleUp-debug-\(stamp).log")
