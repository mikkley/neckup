...
final class AppState: ObservableObject {
    ...
    init() {
        // 品牌更名迁移：旧数据目录/设置域搬到新位置（统计/图鉴/设置不丢）
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        let oldDir = appSupport.appendingPathComponent("NeckUp", isDirectory: true)
        let newDir = appSupport.appendingPathComponent("TurtleUp", isDirectory: true)
        if !FileManager.default.fileExists(atPath: newDir.path),
           FileManager.default.fileExists(atPath: oldDir.path) {
            try? FileManager.default.moveItem(at: oldDir, to: newDir)
        }
        if let old = UserDefaults(suiteName: "com.neckup.mac")?.persistentDomain(forName: "com.neckup.mac") {
    ...
    private func fireBreakReminder() {
        ...
        if settings.gameEnabled {
            ...
            Notifier.send(title: "TurtleUp", body: L10n.breakBody)
        }
    }
