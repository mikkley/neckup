...
- **G-1 休息段微游戏 M1「石斧史莱姆」**：番茄钟休息时岛展开为游戏窗，缓慢点头劈怪（棘轮音渐进提示 + 8-bit 合成音效），60s 一局结算水滴入图鉴（`game.json`），可在设置关闭
...
- 未实现：周/月统计视图（F9）、休息拉伸引导（F10）、勿扰规则（F12）、山峰成长/热力图/成就/微游戏 M2–M5 与图鉴墙 UI（G2–G6 其余部分）
- 头部俯仰 ≠ 探颈，不宣称医学准确
