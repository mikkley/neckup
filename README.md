...
# 无 AirPods：用模拟数据运行（慢速点头梯形波模拟，会周期性触发提醒）
...
- **G-1~G-3 休息段微游戏（五只僵硬怪）**：番茄钟休息时岛展开为游戏窗，洗牌式随机遭遇 60s 一局，结算水滴入图鉴（`game.json`），可在设置关闭
  - M1 石斧史莱姆：缓慢点头劈砍（pitch 25°，棘轮音渐进提示）
  - M2 双头甲虫：左右转头 ≥30° 保持 1.5s 格挡（yaw，前摇预告方向）
  - M3 天平水母：侧屈倾盘接宝石（roll 20°），大宝石悬停定住 5s（连续蓄能音 sonification）
  - M4 气功龟慢慢：收下巴 chin tuck 模式识别（pitch 下探 3–8° 回收保持 5s 蓄力，降级方案为极小幅点头）
  - M5 斜月蝙蝠：低头 10–20° + 转侧 15–25° 复合瞄准，稳定 1s 放箭
- **G-3 成长系统**：水滴浇灌山峰（50 → 青山、200 → 雪峰，连续 3 天无活动枯萎一档，设置里「佛系模式」可关枯萎）；每怪 1/10/30 击败点亮 1/2/3 星；展开态卡片底部有紧凑「成长行」
...
- 未实现：周/月统计视图（F9）、休息拉伸引导（F10）、勿扰规则（F12）、热力图/成就/图鉴墙 UI（G4–G6 其余部分）
- 头部俯仰 ≠ 探颈，不宣称医学准确
